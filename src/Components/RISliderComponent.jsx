...
const RISliderComponent = ({ cards, headingWhite, headingBlack, categories }) => {
  ...
  return (
    <div
      className="container rounded-lg mx-auto mt-20 md:py-20 px-3 md:px-16 border-[2px] border-white font-lexend"
      style={{
        background: "linear-gradient(0deg, #E8F3FF 15.92%, #92A8FF 100%)",
        boxShadow: "0px 4px 24px 0px rgba(0, 69, 197, 0.64)",
      }}
    >
      <h1 className="text-center text-[28px] md:text-4xl md:mb-16 mb-10 md:mt-0 mt-16 font-normal">
        <span className="text-white">{headingWhite}</span> {headingBlack}
      </h1>

      {/* ========= Categories ============ */}
      <nav className="bg-[#FFFFFF66] text-black md:text-[20px] text-[12px] tracking-[4%] flex flex-row md:gap-[20px] gap-[4px] stify-center sm:justify-start items-center mx-auto md:max-w-[682px] sm:max-w-[318px] md:p-[20px] p-[6px] rounded-[3px] overflow-x-auto">
        {categories?.map((item) => (
          <button
            key={item.id}
            className={`px-[8px] py-[4px] font-[600] whitespace-nowrap rounded-lg transition-all duration-200 ${activeCategory === item.cat ? "bg-black text-white" : ""
              }`}
                  ...
                  .map((card, index) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1 my-3 md:ms-8 w-[50%]">
                        <h1 className="font-lexend font-bold md:text-[20px] text-[18px]">
                          {card.name}
                        </h1>
                        <p className="flex flex-row items-center gap-2">
                          <img src="svgIcons/star.svg" alt="Rating" className="" />
                          <span className="md:text-[16px] text-[12px]">{card.rating}</span>
                        </p>
                        <p className="font-lexend font-bold md:text-[16px] text-[12px]">
                          {card.price}
                        </p>
                        <p className="flex flex-row justify-start items-center gap-1 md:text-base md:text-[16px] text-[12px]">
                          <img
      ...
      <div className="flex justify-end items-end gap-4 container md:my-0 my-5">
